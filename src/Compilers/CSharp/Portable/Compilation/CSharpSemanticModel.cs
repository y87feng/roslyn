...
namespace Microsoft.CodeAnalysis.CSharp
{
    ...
    internal abstract class CSharpSemanticModel : SemanticModel
    {
        ...
        /// </summary>
        private (ITypeSymbol type, Nullability nullability) TypeFromVariable(
        ...
        public TypeInfo GetTypeInfo(ExpressionSyntax expression, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            else if (SyntaxFacts.IsDeclarationExpressionType(expression, out DeclarationExpressionSyntax parent))
            {
                switch (parent.Designation.Kind())
                {
                    case SyntaxKind.SingleVariableDesignation:
                        {
                            var declarationInfo = TypeFromVariable((SingleVariableDesignationSyntax)parent.Designation, cancellationToken);
                            var declarationType = (TypeSymbol)declarationInfo.type;
                            return new CSharpTypeInfo(declarationType, declarationType, declarationInfo.nullability, declarationInfo.nullability, Conversion.Identity);
                        }

                    case SyntaxKind.DiscardDesignation:
                        {
                            CSharpTypeInfo declarationInfo = GetTypeInfoWorker(parent, cancellationToken);
                            TypeSymbol declarationType = declarationInfo.Type;
                            return new CSharpTypeInfo(declarationType, declarationType,
                                declarationInfo.Nullability, declarationInfo.ConvertedNullability, Conversion.Identity);
                        }
            ...
            BoundNode boundNodeForSyntacticParent)
        {
            ...
            if (pattern != null)
            {
                ...
                // TODO: Consider language version when returning info
                  ...
                  ((ObjectCreationExpressionSyntax)boundNodeForSyntacticParent.Syntax).Type == boundExpr.Syntax)) // Do not return any type information for a ObjectCreationExpressionSyntax.Type node.
            {
                ...
                Nullability nullability = Nullability.NotComputed, convertedNullability = Nullability.NotComputed;
                ...
                if (nullability != Nullability.NotComputed && convertedNullability == Nullability.NotComputed)
                {
                    convertedNullability = nullability;
                }

                Debug.Assert(nullability != Nullability.NotComputed || convertedNullability == Nullability.NotComputed);
            ...
            Nullability getNullability(Nullability intendedNullability)
            {
                // TODO: Check to ensure C# 8 is enabled and if the type is annotated in source
                // before returning the intended nullability
                return intendedNullability;
            }
        }
        ...
        internal static CSharpTypeInfo GetTypeInfoForSymbol(Symbol symbol)
        {
            ...
            return new CSharpTypeInfo(type, type, Nullability.NotComputed, Nullability.NotComputed, Conversion.Identity);
        }
